...
var PageClick = function (pageclickednumber) {
	...
	$("#size").val(pagenum);
	document.getElementById("vform").action="<c:url value='/admin/rawMaterial/list'/>";
...
$(function () {
	...
	$("#batchFalse").click(function() {
		...
		if (confirm("确定要禁用吗？")) {
			$.ajax({
				url: "<c:url value='/admin/rawMaterial/enabled'/>",
	    		data: {
	    			ids : ids,
	    			enabled : false
	    		},
	...
	$("#batchDel").click(function() {
		...
		if (confirm("确定要删除吗？")) {
			$.ajax({
				url: "<c:url value='/admin/rawMaterial/del'/>",
	    		data: {
	    			ids : ids
	    		},
	    		type: "POST",
...
function view(id) {
	...
	diag.Width = 450;
	diag.Height = 300;
	diag.URL = '<c:url value="/admin/rawMaterial/view-' + id + '"/>';
	diag.Title = "浏览";
	diag.CancelEvent = function () {
		diag.close();
	};
...
function edit(id) {
	...
	diag.Width = 450;
	diag.Height = 300;
	diag.URL = '<c:url value="/admin/rawMaterial/edit-' + id + '"/>';
	diag.Title = "编辑";
	diag.CancelEvent = function () {
		diag.close();
		location.reload();
