...
<script type="text/javascript">
	...
	var PageClick = function (pageclickednumber) {
		...
		$("#size").val(pagenum);
		document.getElementById("vform").action="<c:url value='/admin/sys/user/list'/>";
...
$(function () {
	...
	$("#new").click(function() {
		...
		diag.Width = 450;
		diag.Height = 250;
		diag.URL = '<c:url value="/admin/sys/user/new"/>';
		diag.Title = "新增";
	...
	$("#batchFalse").click(function() {
		...
		if (confirm("确定要禁用吗？")) {
			$.ajax({
				url: "<c:url value='/admin/sys/user/enabled'/>",
	    		data: {
	    			ids : ids,
	    			enabled : false
	    		},
	...
	$("#batchDel").click(function() {
		...
		if (confirm("确定要删除吗？")) {
			$.ajax({
				url: "<c:url value='/admin/sys/user/del'/>",
	    		data: {
	    			ids : ids
	    		},
	    		type: "POST",
...
function resetPWD(id) {
	if (confirm("确定要重置吗？")) {
		$.ajax({
			url: "<c:url value='/admin/sys/user/resetPWD'/>",
    		data: {
    			id : id
    		},
    		type: "POST",
