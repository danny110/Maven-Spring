...
var PageClick = function (pageclickednumber) {
	...
	$("#size").val(pagenum);
	document.getElementById("vform").action="<c:url value='/admin/information/rawMaterial/list'/>";
...
$(function () {
	...
	$("#batchFalse").click(function() {
		...
		if (confirm("确定要禁用吗？")) {
			$.ajax({
				url: "<c:url value='/admin/information/rawMaterial/enabled'/>",
	    		data: {
	    			ids : ids,
	    			enabled : false
	    		},
	...
	$("#batchDel").click(function() {
		...
		if (confirm("确定要删除吗？")) {
			$.ajax({
				url: "<c:url value='/admin/information/rawMaterial/del'/>",
	    		data: {
	    			ids : ids
	    		},
	    		type: "POST",
...
$("#new").click(function() {
	...
	diag.Width = 450;
	diag.Height = 300;
	diag.URL = '<c:url value="/admin/information/rawMaterial/new"/>';
	diag.Title = "新增";
...
function view(id) {
	...
	diag.Width = 450;
	diag.Height = 300;
	diag.URL = '<c:url value="/admin/information/rawMaterial/view-' + id + '"/>';
	diag.Title = "浏览";
	diag.CancelEvent = function () {
		diag.close();
	};
...
function edit(id) {
	...
	diag.Width = 450;
	diag.Height = 300;
	diag.URL = '<c:url value="/admin/information/edit-' + id + '"/>';
	diag.Title = "编辑";
	diag.CancelEvent = function () {
		diag.close();
		location.reload();
