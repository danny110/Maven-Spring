...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		if( config.icon && !isIfr )
		{
			...
			DOM.icon[0].innerHTML = '<img src="' + webRootPath + config.path + 'skins/icons/' + config.icon + '" class="ui_icon_bg"/>';
		}
		else
...
lhgdialog.setting =
{
    ...
    path: '/resources/images/',           // lhgdialog路径
	lock: false,				// 是否锁屏
 ...
 * @param	{String}	消息内容
 */
lhgdialog.alert = function( content, callback, skin, parent )
{
	return lhgdialog({
		title: '警告',
		id: 'Alert',
 ...
 * @param	{String}	消息内容
 */
lhgdialog.Tips = function( content, callback, skin, parent )
{
	return lhgdialog({
		title: '提示',
		id: 'Tips',
 ...
 */
lhgdialog.confirm = function( content, yes, no,skin, parent )
{
	return lhgdialog({
		title: '确认',
		id: 'confirm',
 ...
 * @param	{String}	默认值
 */
lhgdialog.prompt = function( content, yes, value,skin, parent )
{
	value = value || '';
	var input;
 ...
 */
lhgdialog.tips = function(content, time, icon, callback,skin)
{
	var reIcon = icon ? function(){
		this.DOM.icon[0].innerHTML = '<img src="' + webRootPath  + this.config.path + 'skins/icons/' + icon + '" class="ui_icon_bg"/>';
