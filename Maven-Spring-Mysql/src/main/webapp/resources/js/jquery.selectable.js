	...
	jQselectable.prototype = {
		...
		build: function() {
			...
			if ( !this.m_input ) {
				...
				this.m_input.append(this.m_text).attr({
					...
					href: '#'
				}).css('width',this.conf.width).addClass('sctble_display').addClass(_style).addClass(this.attrs.cl).insertAfter(this.target);
				this.target.hide();
				this.mat = $('<div/>');
				// Customized
				if ( _style == 'simple' ) {
		...
		bind_events: function() {
			...
			var set_pos = function() {
				...
				var matWidth=_this.conf.maskIco?_this.m_input.width():_this.m_input.width()-_this.conf.icoWidth;
				_this.mat.css({
					top: topPos,
