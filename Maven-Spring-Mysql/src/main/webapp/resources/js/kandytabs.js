(function($) {
    $.fn.KandyTabs = function(n) {
        ...
        var p = document.URL.toLowerCase() + "&",
        $tab,
        $title,
        $body,
        $btn,
        ...
        i,
        n = $.extend(o, n);
        if (typeof n.ready == "function") n.ready();
        this.each(function() {
            if ($(this).data("KandyTabs") == true) return false;
            var c = n.current - 1,
            _except = n.except,
            _child = n.child[0],
            _childOptions = n.child[1],
            _stall = n.stall,
            _last = n.last,
            _col = n.column,
            $this = $(this).data("KandyTabs", true),
            $child = $this.children(),
            _childlen = $child.length,
            _process = false,
            _tagname = this.tagName,
            _tag = "div";
            if (n.process && n.auto) _process = true;
            if (_tagname == "DL") {
                $title = $("<dt/>");
                $body = $("<dd/>");
                _tag = "dd"
            ...
            if (n.btn == "" && n.cont == "") {
                if (n.type == "slide") {
                    ...
                    if ($tab.find(".tabbtn").length > $tab.find(".tabcont").length) $tab.append('<' + _tag + ' class="tabcont">' + n.lang.empty + '</' + _tag + '>')
                } else {
                    _childlen = _childlen / 2;
                    if (_childlen.toString().indexOf(".") > -1) _childlen = parseInt(_childlen) + 1;
                    for (i = 0; i < _childlen; i++) {
                        ...
                        if (_tmpb.tagName == "A" || _tmpb.tagName == "IMG") {
                            ...
                            if (n.type != "fold") $child.eq(i * 2).remove()
                        }
                        ...
                        if (_tmpc) {
                            if (_tmpc.tagName == "A" || _tmpc.tagName == "IMG" || _tmpc.tagName == "UL" || _tmpc.tagName == "OL" || _tmpc.tagName == "DL" || _tmpc.tagName == "IFRAME") {
                                ...
                                $tmpcont = $tmpcont.wrap('<div class="tabcont' + (_tmpc.className ? ' ' + _tmpc.className + '': '') + '"' + (_tmpc.id ? ' id="' + _tmpc.id + '"': '') + '/>').parent().html($tmpcont.html())
                            } else {
                                $tmpcont.addClass("tabcont")
                            }
                        } else {
            ...
            if (n.type != "fold") {
                ...
                $cont.eq(c).show();
                if (n.direct == "left") $cont.css("float", "left")
            } else {
                if (n.direct == "left") {
                    var f = $cont.width();
                    $tab.css({
                ...
                if (n.action == "roll") n.action = "slide";
                if (n.action == "slifade") n.action = "fade"
            }
            var g = function(a, b) {
                if (!n.resize) return false;
            ...
            var h = function(a) {
                ...
                _tabprev = _tabprev || "",
                _tabindex = "";
                if ($tabnext) b = $tabnext.index();
                if ($tabprev) _tabprev = $tabprev.index();
                if (b != _tabprev && b == (a == 0 ? -1 : a)) {
                    if (a == 0) _tabindex = 1
                }
                if (b != _tabprev && _tabprev == (a == (_childlen - 1) ? -1 : a)) {
                    if (a == (_childlen - 1)) _tabindex = -1
                }
                switch (n.direct) {
                case "left":
            ...
            var j = function(a) {
                $cont.eq(a).stop(false, true).fadeIn(0,
                function() {
            ...
            if (n.action == "roll") {
                ...
                if (n.loop) $roll.append($roll.html());
                if (n.direct == "left") {
                    var k = 0;
            ...
            var l = function(a) {
                ...
                if (typeof n.custom == "function") n.custom($btn, $cont, a, $this);
                if ($prev) a == 0 ? $prev.addClass("tabprevno").attr("title", n.lang.first[0]) : $prev.removeClass("tabprevno").attr("title", n.lang.prev[0]);
                if ($next) a == (_childlen - 1) ? $next.addClass("tabnextno").attr("title", n.lang.last[0]) : $next.removeClass("tabnextno").attr("title", n.lang.next[0]);
                if ($now) $now.text(a + 1);
                if (n.loop && n.nav) $prev.removeClass("tabprevno").attr("title", n.lang.prev[0]),
                $next.removeClass("tabnextno").attr("title", n.lang.next[0])
            };
            _auto = function() {
                setAuto && clearTimeout(setAuto);
                setAuto = null;
                window.CollectGarbage && CollectGarbage();
                if (n.process) _isProcess = true;
                ...
                if (n.trigger == "mouseover") if (n.process) $process.stop().width("").animate({
                    width: 0
                },
                _stall);
            ...
            if (n.auto) {
                ...
                $autostop.mouseover(function() {
                    if (n.process) $process.stop().width(""),
                    _isProcess = false;
                    clearTimeout(setAuto)
                }).mouseout(function() {
                    if (n.process) $process.stop().width("").animate({
                        width: 0
                    },
                    _stall),
                    _isProcess = true;
                ...
                if (n.ctrl) {
                    ...
                    $ctrl.append($pause = $('<b class="tabpause" title="' + n.lang.pause[0] + '">' + n.lang.pause[1] + '</b>'), $play = $('<b class="tabplay" title="' + n.lang.play[0] + '" style="display:none">' + n.lang.play[1] + '</b>'));
                    $pause.click(function() {
                        $(this).hide();
                        if (n.process) $process.stop().hide();
                        clearTimeout(setAuto);
                        $play.show();
                        _isAuto = false
                    });
                    $play.click(function() {
                        $(this).hide();
                        if (n.process) $process.show().stop().width("").animate({
                            width: 0
                        },
                        _stall);
                        setAuto = setTimeout(_auto, _stall);
            ...
            if (n.nav) {
                ...
                $nav.append($prev = $('<em class="tabprev" title="' + n.lang.prev[0] + '">' + n.lang.prev[1] + '</em>'), '<span class="tabpage"/>', $next = $('<em class="tabnext" title="' + n.lang.next[0] + '">' + n.lang.next[1] + '</em>'));
                $nav.find("span.tabpage").append($now = $('<b class="tabnow">' + (c + 1) + '</b>'), '<i>&nbsp;/&nbsp;</i>', '<b class="taball">' + _childlen + '</b>');
                if (c == 0 && !n.loop) $prev.addClass("tabprevno");
                if (c == (_childlen - 1) && !n.loop) $next.addClass("tabnextno");
                $prev.mouseover(function() {
                    if (_process) $process.stop().width(""),
                    _isProcess = false;
                    if (n.auto) clearTimeout(setAuto)
                }).mousedown(function() {
                    ...
                    if (n.auto) setAuto = setTimeout(_auto, _stall)
                }).mouseup(function() {
                    if (n.auto) clearTimeout(setAuto)
                }).mouseout(function() {
            ...
            var m = null;
            if (n.trigger != "mouseover") n.delay = 0;
            $btn.bind(n.trigger,
            function() {
            ...
            if (typeof n.done == "function") n.done($btn, $cont, $this)
        })
    }
})(jQuery);